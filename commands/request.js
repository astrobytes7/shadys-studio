...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('request')
        .setDescription('Request related commands.')
        .addSubcommand(subcommand =>
            subcommand
                .setName('payout')
                .setDescription('Request a payout.')
    ...
    async execute(interaction) {
        if (interaction.options.getSubcommand() === 'payout') {
            ...
            try {
                ...
                const embed = new EmbedBuilder()
                    .setColor("#242429")
                    .setTitle(`New Payout Request Logged`)
                    .setAuthor({
                ...
                await interaction.reply({ content: `Your payout request has been logged successfully.`, ephemeral: true });

            } catch (error) {
                console.error('Error executing command:', error);
                await interaction.reply({ content: 'There was an error logging the payout request. Please try again later.', ephemeral: true });
            }
        }
    },
};
